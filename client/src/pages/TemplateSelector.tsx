...
const TemplateSelector: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'modern', name: 'Modern', count: RESUME_TEMPLATES.filter(t => t.category === 'modern').length },
    { id: 'professional', name: 'Professional', count: RESUME_TEMPLATES.filter(t => t.category === 'professional').length },
    { id: 'creative', name: 'Creative', count: RESUME_TEMPLATES.filter(t => t.category === 'creative').length }
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-center">
            <ol className="flex items-center w-full max-w-3xl">
              ...
              <div className={`flex-1 h-0.5 mx-4 ${templateSelectionStep >= 2 ? 'bg-indigo-600 dark:bg-indigo-400' : 'bg-gray-300 dark:bg-gray-600'}`}></div>
              <li className={`flex items-center ${templateSelectionStep >= 2 ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
                <span className={`flex items-center justify-center w-8 h-8 rounded-full border-2 ${
                  templateSelectionStep >= 2 ? 'border-indigo-600 dark:border-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'border-gray-500 dark:border-gray-600'
                }`}>
                  2
                </span>
                <span className="ml-2 text-sm font-medium">Customize Resume</span>
              </li>
              ...
              <li className="flex items-center text-gray-500 dark:text-gray-400">
                ...
                <span className="ml-2 text-sm font-medium">Download & Share</span>
              </li>
            </ol>
          </div>
        </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
              Select one of our professional templates. All are ATS-optimized with real-time preview and job matching features.
            </p>
          </div>
            ...
            {categories.map((category) => (
              ...
              >
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${
                  selectedCategory === category.id ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
                }`}>
                  {category.count}
                </span>
              </button>
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                      template.category === 'modern' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' :
                      template.category === 'professional' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' :
                      template.category === 'creative' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300' :
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {template.features.slice(0, 2).map((feature, index) => (
                      <span key={index} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded font-medium">
                        {feature}
                      </span>
                    ))}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden mb-8 border dark:border-gray-700">
              <div className="aspect-w-4 aspect-h-3 bg-gray-50 dark:bg-gray-900 overflow-hidden">
                <img 
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = 'https://via.placeholder.com/600x400?text=Template+Preview';
                  }}
                />
              </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1.5 text-sm rounded-full font-medium ${
                    selectedTemplate?.category === 'modern' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' :
                    selectedTemplate?.category === 'professional' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' :
                    selectedTemplate?.category === 'creative' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300' :
                    'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'
                  }`}>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {selectedTemplate?.features.map((feature, index) => (
                    <span key={index} className="text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1 rounded font-medium">
                      {feature}
                    </span>
                  ))}
                </div>
                ...
                {/* Recommended for */}
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Perfect for</h3>
                <div className="flex flex-wrap gap-2 mb-8">
                  {selectedTemplate?.recommended.map((item, index) => (
                    <span key={index} className="text-sm bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 px-3 py-1 rounded font-medium">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/30 rounded-lg p-4 text-sm text-yellow-800 dark:text-yellow-300">
              <div className="flex">
                <svg className="w-5 h-5 text-yellow-600 dark:text-yellow-500 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p>
                  <strong>Tip:</strong> Our templates are all ATS-optimized, but remember that clean, well-structured content matters most for getting past Applicant Tracking Systems.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 dark:bg-gray-800">
            ...
            <div className="bg-gray-100 dark:bg-gray-900 p-6 rounded-lg mb-6 min-h-[500px] flex flex-col items-center justify-center">
              {error ? (
                ...
                <Document
                  file={previewTemplate.pdfPreview || previewTemplate.thumbnail}
                  ...
                  loading={
                    <div className="flex flex-col items-center justify-center py-10">
                      <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-indigo-600 dark:border-indigo-400 mb-3"></div>
                  ...
                  noData={
                    <div className="text-center">
                      <img 
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = 'https://via.placeholder.com/600x800?text=Template+Preview';
                        }}
                      />
                    </div>
                  }
              ...
              {numPages && numPages > 1 && (
                <div className="flex justify-center items-center mt-6 space-x-4">
                  <button 
                    ...
                    className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-200"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
